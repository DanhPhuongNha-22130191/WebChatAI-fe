...
const normalizeMessage = (raw) => {
  ...
  const isRecalled = raw.recalled === true || raw.status === "recalled";
  ...
  const createdAt =
    raw.createAt ?? raw.createdAt ?? raw.time ?? new Date().toISOString();
...
export const handleSendChat = (
  ...
  if (response.mes === "Message sent") {
    ...
    dispatch(updateSidebar(normalizedMessage));
...
export const handleRecallMessage = (
  ...
  if (!recalledMessage || !recalledMessage.id) {
    console.error("Dữ liệu tin nhắn thu hồi không hợp lệ:", response.data);
    return;
  }
...
export const handleEditMessage = (
  ...
  if (response.status !== "success" && response.status !== true) {
    console.error("Chỉnh sửa tin nhắn thất bại:", response.mes || response);
  ...
  if (!editedMessage || !editedMessage.id) {
    console.error("Dữ liệu tin nhắn chỉnh sửa không hợp lệ:", response.data);
    return;
  }
